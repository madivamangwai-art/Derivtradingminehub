...
export const getSpinData = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [tickets, history] = await Promise.all([
      supabase
        .from("spin_tickets")
        .select("id,value_kes,source,created_at")
        .eq("user_id", userId)
        .is("used_at", null)
        .order("created_at"),
      supabase
        .from("spin_tickets")
        .select("value_kes,prize_amount,prize_label,used_at")
        .eq("user_id", userId)
        .not("used_at", "is", null)
    ...
    (tickets.data ?? []).forEach((t) => {
      counts[t.value_kes] = (counts[t.value_kes] ?? 0) + 1;
    });
    return {
      ticketCounts: counts,
...
export const buyTickets = createServerFn({ method: "POST" })
    ...
    z
      .object({
        value: z
          .number()
          .int()
          .refine((v) => TICKET_VALUES.includes(v)),
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await supabaseAdmin.rpc("buy_spin_tickets_atomic", {
      _user_id: userId,
      _value_kes: data.value,
      _qty: data.qty,
    });
    if (error) throw new Error("Insufficient wallet balance");
    return { ok: true };
  });
...
export const spinTicket = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (roll < 0.001)
      idx = PATTERN.length + 2; // 10x
    else if (roll < 0.001 + 1 / 300)
      idx = PATTERN.length + 1; // 5x
    else if (roll < 0.001 + 1 / 300 + 1 / 200) idx = PATTERN.length; // 3x

    const slot = WHEEL[idx];
    const prize = Math.floor(slot.mult * data.value);
    const { data: usedTicket, error: useError } = await supabaseAdmin
      .from("spin_tickets")
      .update({
        used_at: new Date().toISOString(),
        prize_amount: prize,
        prize_label: slot.label,
      })
      .eq("id", t.id)
      .is("used_at", null)
      .select("id")
    ...
    if (!usedTicket) throw new Error("Ticket was already used. Refresh and try again.");
    if (slot.free) {
      await supabaseAdmin
        .from("spin_tickets")
        .insert({ user_id: userId, value_kes: data.value, source: "free_spin" });
    }
    if (prize > 0) {
      await supabaseAdmin.rpc("adjust_wallet_atomic", {
        _user_id: userId,
        _balance_delta: prize,
        _earned_delta: prize,
        _deposited_delta: 0,
        _withdrawn_delta: 0,
        _require_sufficient_balance: false,
      });
      await supabaseAdmin.from("transactions").insert({
        user_id: userId,
        kind: "spin_win",
        amount: prize,
        description: `Spin win (${slot.label} on ${data.value})`,
      });
    }
