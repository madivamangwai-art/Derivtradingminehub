...
import { DEPOSIT_SURCHARGE_RATE, getDepositPromptAmount, initiateStkPush } from "@/lib/mpesa.functions";
          ...
          <div className="space-y-3">
            ...
            {depositPromptAmount > 0 && (
              <div className="rounded-lg bg-muted/40 px-3 py-2 text-xs">
                <div className="flex justify-between"><span className="text-muted-foreground">Wallet credit</span><span>{fmt(depositAmount)}</span></div>
                <div className="mt-1 flex justify-between"><span className="text-muted-foreground">5% service addition</span><span>{fmt(depositSurcharge)}</span></div>
                <div className="mt-1 flex justify-between border-t border-border/60 pt-1 font-medium"><span>M-Pesa prompt</span><span>{fmt(depositPromptAmount)}</span></div>
              </div>
            )}
            <Button onClick={() => deposit.mutate()} disabled={deposit.isPending || !depAmt || !phone} className="w-full gradient-gold">
              {deposit.isPending ? "Sending prompt..." : depositPromptAmount > 0 ? `Prompt ${fmt(depositPromptAmount)}` : "Send M-Pesa STK Push"}
            </Button>
            <p className="text-[11px] text-muted-foreground">
              The prompt includes a {Math.round(DEPOSIT_SURCHARGE_RATE * 100)}% addition, but your wallet is credited with the amount you entered.
            </p>
          </div>
        </TabsContent>
