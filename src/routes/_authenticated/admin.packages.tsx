        ...
        <div className="glass-card overflow-hidden rounded-2xl">
          ...
          {(data ?? []).map((p: any) => (
            <button
              key={p.id}
              onClick={() => setForm(p)}
              className="grid w-full grid-cols-[80px_1fr_100px_100px_80px_60px] items-center gap-2 border-b border-border/40 px-4 py-3 text-left text-sm hover:bg-muted/40"
            >
