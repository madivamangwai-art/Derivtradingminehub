...
function SupportAdmin() {
  ...
  useEffect(() => {
    if (data?.settings) {
      setWa(data.settings.whatsapp_url ?? "");
      setTg(data.settings.telegram_url ?? "");
    }
  }, [data?.settings]);
  ...
  useEffect(() => {
    if (!selected && threads[0]) setSelected(threads[0].user_id);
  }, [threads, selected]);
  ...
  return (
    <AdminShell title="Support">
      <div className="glass-card rounded-2xl p-5">
        ...
        <Button onClick={() => saveLinks.mutate()} disabled={saveLinks.isPending} className="mt-3">
          Save links
        </Button>
      </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[280px_1fr]">
        <div className="glass-card rounded-2xl p-2">
          <div className="border-b border-border/40 px-3 py-2 text-xs font-semibold uppercase text-muted-foreground">
            Threads
          </div>
          <div className="max-h-[60vh] overflow-y-auto">
            ...
            {threads.map((t) => (
              <button
                ...
                className={`block w-full rounded-lg px-3 py-2 text-left text-sm ${selected === t.user_id ? "bg-primary/15" : "hover:bg-muted"}`}
              >
        ...
        <div className="glass-card flex h-[65vh] flex-col rounded-2xl p-4">
          <div className="flex-1 space-y-2 overflow-y-auto">
            {(active?.messages ?? []).map((m: any) => (
              <div
                key={m.id}
                className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${m.sender === "admin" ? "ml-auto bg-primary text-primary-foreground" : "bg-muted"}`}
              >
                <div>{m.message}</div>
