...
import { markWithdrawalSuccess } from "@/lib/payment-reconcile";
      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          const originatorConversationId =
            payload?.OriginatorConversationID ?? payload?.Result?.OriginatorConversationID;
          ...
          if (conversationId)
            query = query.or(
              `conversation_id.eq.${conversationId},originator_conversation_id.eq.${conversationId}`,
            );
          if (originatorConversationId)
            query = query.or(
              `conversation_id.eq.${originatorConversationId},originator_conversation_id.eq.${originatorConversationId}`,
            );
          ...
          if (String(resultCode) === "0") {
            await markWithdrawalSuccess(supabaseAdmin, {
              ...
              metadata: payload,
              providerReference: transactionId ?? conversationId ?? null,
              resultDesc: resultDesc ? String(resultDesc) : "Payout completed.",
            });
          } else {
            await supabaseAdmin
              .from("withdrawals")
